...
echo "=== Complete VPS DNS Fix for Discord Bot ==="
echo
...
# Add all necessary domains to /etc/hosts as fallback
echo "3. Adding service domains to /etc/hosts as fallback..."
# Backup hosts file
...
sed -i '/discordapp\.net/d' /etc/hosts
sed -i '/discordstatus\.com/d' /etc/hosts
sed -i '/googleapis\.com/d' /etc/hosts
sed -i '/youtube\.com/d' /etc/hosts
sed -i '/youtubei\.googleapis\.com/d' /etc/hosts
sed -i '/odysee\.tv/d' /etc/hosts
sed -i '/rumble\.com/d' /etc/hosts
...
162.159.137.232 cdn.discordapp.com
162.159.137.232 media.discordapp.net
104.21.58.190 discordstatus.com

# Google APIs
172.217.1.138 www.googleapis.com
172.217.1.138 youtube.googleapis.com
172.217.1.138 youtubei.googleapis.com
172.217.1.142 www.youtube.com

# Platform APIs
104.21.12.139 api.odysee.tv
104.26.9.156 api.rumble.com
EOF
echo "Service domains added to /etc/hosts"
echo
...
echo -n "Host DNS test (YouTube API): "
nslookup youtube.googleapis.com >/dev/null 2>&1 && echo "SUCCESS" || echo "FAILED"

echo -n "Host DNS test (Odysee): "
nslookup api.odysee.tv >/dev/null 2>&1 && echo "SUCCESS" || echo "FAILED"
...
if docker run --rm alpine nslookup youtube.googleapis.com >/dev/null 2>&1; then
    echo "Docker DNS test (YouTube API): SUCCESS"
else
    echo "Docker DNS test (YouTube API): FAILED (this is expected if using hosts file fallback)"
fi
echo
...
echo "=== Complete DNS Fix Applied ==="
echo
echo "All necessary domains have been added to both:"
echo "1. Docker container configuration (via extra_hosts in docker-compose.yml)"
echo "2. Host /etc/hosts file (as system-wide fallback)"
echo "3. Docker daemon DNS configuration"
echo
echo "This triple-layer approach ensures connectivity even if DNS servers fail."
echo
