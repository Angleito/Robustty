...
class HealthMonitor:
    ...
    def __init__(self, bot, config: Dict[str, Any]):
        ...
        self.max_consecutive_failures = self.health_config.get('max_consecutive_failures', 3)
        ...
        self.use_exponential_backoff = self.recovery_config.get('exponential_backoff', True)
        self.max_recovery_delay = self.recovery_config.get('max_delay', 300)  # 5 minutes
    ...
    async def _run_all_health_checks(self):
        ...
        service_names.extend([f'platform_{name}' for name in self.bot.platform_registry.get_enabled_platforms().keys()])
        ...
        try:
            ...
            for guild_id, player in self.bot.audio_players.items():
                if player.voice_client and player.voice_client.is_connected():
                    ...
                    if not player.voice_client.is_playing() and player.queue and len(player.queue) > 0:
        ...
        try:
            ...
            try:
                ...
                if any(term in error_str for term in ['timeout', 'connection', 'network']):
                    ...
                    status = ConnectionStatus.DEGRADED
                elif any(term in error_str for term in ['auth', 'key', 'token', '401', '403']):
                    status = ConnectionStatus.UNHEALTHY
                else:
                    status = ConnectionStatus.DEGRADED
    ...
    async def _process_health_result(self, service_name: str, result: HealthCheckResult):
        ...
        if health.avg_response_time == 0:
            ...
            health.avg_response_time = (health.avg_response_time * 0.8) + (result.response_time * 0.2)
        ...
        self.health_check_duration.labels(service=service_name).observe(result.response_time)
        self.consecutive_failures_gauge.labels(service=service_name).set(health.consecutive_failures)
        
        # Log health status changes
        if result.status != ConnectionStatus.HEALTHY:
            log_level = logging.WARNING if result.status == ConnectionStatus.DEGRADED else logging.ERROR
            logger.log(
                log_level,
        ...
        if (health.consecutive_failures >= self.max_consecutive_failures and 
            result.status == ConnectionStatus.UNHEALTHY):
    ...
    async def _trigger_recovery(self, service_name: str, health: ConnectionHealth):
        ...
        self.recovery_attempts_total.labels(service=service_name, status="started").inc()
        
        logger.info(f"Triggering recovery for {service_name} (attempt {health.recovery_attempts})")
    ...
    async def _recover_service(self, service_name: str, health: ConnectionHealth):
        ...
        try:
            ...
            if self.use_exponential_backoff:
                delay = min(self.max_recovery_delay, 10 * (2 ** (health.recovery_attempts - 1)))
            else:
            ...
            if success:
                ...
                self.recovery_attempts_total.labels(service=service_name, status="success").inc()
                health.recovery_attempts = 0
            else:
                logger.warning(f"Recovery attempt failed for {service_name}")
                self.recovery_attempts_total.labels(service=service_name, status="failed").inc()
                
        except Exception as e:
            logger.error(f"Recovery error for {service_name}: {e}", exc_info=True)
            self.recovery_attempts_total.labels(service=service_name, status="error").inc()
            ...
            for guild_id, player in self.bot.audio_players.items():
                if player.voice_client and not player.voice_client.is_connected():
                    try:
                        ...
                        logger.warning(f"Failed to recover voice connection for guild {guild_id}: {e}")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for service_name, health in self.connection_health.items():
            ...
            elif health.status == ConnectionStatus.DEGRADED and overall_status == ConnectionStatus.HEALTHY:
