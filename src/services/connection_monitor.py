...
class ConnectionMonitor:
        ...
        try:
            for guild_id, voice_client in list(self.bot.voice_clients_by_guild.items()):
                if voice_client and not voice_client.is_connected():
                    logger.warning(
                        f"Detected disconnected voice client for guild {guild_id}"
                    )
    ...
    async def _handle_disconnected_voice_client(
        ...
        try:
            ...
            logger.info(
                f"Attempting to reconnect to {last_channel.name} in guild {guild_id} (attempt {attempts + 1})"
            )
            ...
            try:
                ...
                # Reconnect
                new_voice_client = await last_channel.connect(
                ...
                if hasattr(self.bot, "audio_players"):
                    ...
                    if audio_player:
                        ...
                        logger.info(
                            f"Updated audio player voice client for guild {guild_id}"
                        )

                # Reset reconnection attempts on success
                self.reconnection_attempts[guild_id] = 0
                logger.info(
                    f"Successfully reconnected to {last_channel.name} in guild {guild_id}"
                )
                ...
                logger.error(
                    f"Reconnection attempt {attempts + 1} failed for guild {guild_id}: {e}"
                )
            ...
            logger.error(
                f"Error handling disconnected voice client for guild {guild_id}: {e}"
            )
...
class StreamHealthMonitor:
    ...
    def get_health_stats(self) -> Dict:
        ...
        return {
            ...
            "failure_rate": len(self.failed_urls)
            / max(1, len(self.url_failure_counts)),
        }
