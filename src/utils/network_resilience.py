        ...
        async def wrapper(*args, **kwargs) -> T:
            ...
            for attempt in range(1, config.max_attempts + 1):
                try:
                    ...
                    logger.warning(
                        f"Function {func.__name__} attempt {attempt} failed: {e}. Retrying in {delay:.2f}s"
                    )
...
def with_circuit_breaker(
    service_name: Optional[str] = None, config: Optional[CircuitBreakerConfig] = None
):
    """
...
async def safe_aiohttp_request(
    session: aiohttp.ClientSession, method: str, url: str, timeout: int = 30, **kwargs
...
REDIS_CIRCUIT_BREAKER_CONFIG = CircuitBreakerConfig(
    failure_threshold=5, recovery_timeout=60, success_threshold=3, timeout=10
)

REDIS_RETRY_CONFIG = RetryConfig(
    max_attempts=2, base_delay=0.5, max_delay=5.0, exponential_base=2.0, jitter=True
)
