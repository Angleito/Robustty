...
class StructuredLogger(logging.LoggerAdapter):
    ...
    def process(self, msg: str, kwargs: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
        ...
        if "start_time" in combined_extra:
            combined_extra["duration_ms"] = int(
                (time.time() - combined_extra["start_time"]) * 1000
            )
    ...
    def log_operation_complete(
        self, operation: str, context: Dict[str, Any], **kwargs
    ) -> None:
        """Log the completion of an operation with timing."""
        self.info(f"Completed {operation} operation", extra={**context, **kwargs})

    def log_operation_error(
        self, operation: str, error: Exception, context: Dict[str, Any], **kwargs
    ) -> None:
...
class RumbleExtractor:
    ...
    def __init__(
        ...
        if self.api_token and ApifyClient:
            ...
            logger.warning(
                "apify_client package not installed - Rumble extraction will not work"
            )
            self.client = None
        else:
            self.client = None
        ...
        if not self.api_token:
            self.logger.warning(
                "APIFY_API_TOKEN not found. Rumble extraction may fail."
            )
    ...
    async def get_video_metadata(self, url: str) -> Dict[str, Any]:
        ...
        context = log.log_operation_start("metadata", video_id=video_id)

        try:
            if not self.api_token:
                raise PlatformAuthenticationError(
                    "API token is required for metadata extraction", platform="Rumble"
                )
            ...
            if self.cache_manager and video_id != "unknown":
                cached_metadata = await self.cache_manager.get_video_metadata(
                    "rumble", video_id
                )
                if cached_metadata:
                    self.metrics.record_cache_hit("metadata")
                    log.info(
                        "Retrieved metadata from cache", extra={"video_id": video_id}
                    )
                    log.log_operation_complete(
                        "metadata", context, has_metadata=True, from_cache=True
                    )
            ...
            actor_input = {"urls": [url], "proxyConfig": {"useApifyProxy": True}}

            # Call the Apify actor
            result = await self._make_actor_call(
                "junglee/rumble-video-extractor", actor_input
            )
            ...
            if result and "items" in result and len(result["items"]) > 0:
                ...
                if self.cache_manager and video_id != "unknown":
                    await self.cache_manager.set_video_metadata(
                        "rumble", video_id, metadata
                    )
                    log.info("Cached metadata", extra={"video_id": video_id})

                log.log_operation_complete(
                    "metadata", context, has_metadata=True, from_cache=False
                )
                return metadata
            else:
                raise PlatformNotAvailableError(
                    f"No metadata found for video: {url}", platform="Rumble"
                )
    ...
    async def download_audio(self, url: str, quality: str = "best") -> str:
        ...
        context = log.log_operation_start(
            "download", video_id=video_id, quality=quality
        )

        try:
            if not self.api_token:
                raise PlatformAuthenticationError(
                    "API token is required for audio extraction", platform="Rumble"
                )
            ...
            if self.cache_manager and video_id != "unknown":
                cached_stream_url = await self.cache_manager.get_stream_url(
                    "rumble", video_id, quality
                )
                if cached_stream_url:
                    self.metrics.record_cache_hit("stream")
                    log.info(
                        "Retrieved stream URL from cache",
                        extra={"video_id": video_id, "quality": quality},
                    )
                    log.log_operation_complete(
                        "download", context, has_stream=True, from_cache=True
                    )
            ...
            if result and "items" in result and len(result["items"]) > 0:
                ...
                if quality == "best" and "videoUrl" in item:
                    ...
                    raise PlatformNotAvailableError(
                        "No stream URL found in video data", platform="Rumble"
                    )

                if stream_url:
                    # Cache the stream URL if cache manager is available
                    if self.cache_manager and video_id != "unknown":
                        await self.cache_manager.set_stream_url(
                            "rumble", video_id, stream_url, quality
                        )
                        log.info(
                            "Cached stream URL",
                            extra={"video_id": video_id, "quality": quality},
                    ...
                    log.log_operation_complete(
                        "download", context, has_stream=True, from_cache=False
                    )
                    return stream_url
                else:
                    raise PlatformNotAvailableError(
                        "Stream URL is empty", platform="Rumble"
                    )
            else:
                raise PlatformNotAvailableError(
                    f"No download data found for video: {url}", platform="Rumble"
                )
    ...
    async def search_videos(
        ...
        try:
            if not self.api_token:
                raise PlatformAuthenticationError(
                    "API token is required for search operations", platform="Rumble"
                )

            # Check cache first for search results
            if self.cache_manager:
                cached_results = await self.cache_manager.get_search_results(
                    "rumble", query
                )
                if cached_results and len(cached_results) >= max_results:
                    log.info(
                        "Retrieved search results from cache", extra={"query": query}
                    )
                    log.log_operation_complete(
                        "search",
                        context,
                        result_count=len(cached_results[:max_results]),
                        from_cache=True,
                    )
            ...
            if self.cache_manager and videos:
                ...
                log.info(
                    ...
                    extra={"query": query, "count": len(videos)},
                )

            log.log_operation_complete(
                "search", context, result_count=len(videos), from_cache=False
            )
            return videos
    ...
    async def _make_actor_call(
        ...
        log = self.logger.with_context(
            actor_id=actor_id, input_size=len(str(input_data))
        )
        ...
        try:
            ...
            result = await asyncio.wait_for(actor_task, timeout=timeout_seconds)

            log.log_operation_complete(
                "actor_call", context, result_size=len(str(result))
            )
            return result

        except asyncio.TimeoutError:
            raise PlatformNotAvailableError(
                f"Request timed out after {self.actor_timeout}ms", platform="Rumble"
            )

        except asyncio.CancelledError:
            log.warning("Actor call was cancelled", extra=context)
            raise PlatformError("Request was cancelled", platform="Rumble")
            ...
            if "rate limit" in error_msg or "429" in error_msg:
                raise PlatformRateLimitError(
                    "API rate limit exceeded", platform="Rumble", original_error=e
                )
            elif "unauthorized" in error_msg or "401" in error_msg:
                raise PlatformAuthenticationError(
                    "Authentication failed", platform="Rumble", original_error=e
                )
            else:
                raise PlatformAPIError(
                    f"Actor call failed: {str(e)}", platform="Rumble", original_error=e
                )
    ...
    async def _call_apify_actor(
        ...
        if not self.client:
            raise PlatformAuthenticationError(
                "Apify client not initialized. API token required.", platform="Rumble"
            )
        ...
        try:
            ...
            run = await asyncio.get_event_loop().run_in_executor(
                None, lambda: actor.call(input_json=input_data, wait_secs=timeout_secs)
            )
            ...
            if run and run.get("defaultDatasetId"):
                ...
                items = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: list(dataset.iterate_items())
                )
                return {"items": items}
            else:
                return {}
