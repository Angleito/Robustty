...
class CrossPlatformCookieExtractor:
    ...
    def extract_all_cookies(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(self.browsers)
        ) as executor:
            future_to_browser = {
                executor.submit(
                    self._extract_browser_cookies, browser, domains
                ): browser
    ...
    def _extract_browser_cookies(
        self, browser: str, domains: Optional[List[str]] = None
    ) -> List[Cookie]:
    ...
    def save_cookies_json(
        self, output_path: Path, domains: Optional[List[str]] = None
    ) -> None:
    ...
    try:
        ...
        for platform in platforms:
            try:
                ...
                logger.info(
                    f"Saved {len(cookie_data)} {platform} cookies to {output_file}"
                )
