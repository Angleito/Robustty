...
from ..utils.embeds import create_embed  # type: ignore
...
class Info(commands.Cog):
    ...
    async def help(
        self, ctx: commands.Context[commands.Bot], command_name: Optional[str] = None
    ):
    ...
    async def uptime(self, ctx: commands.Context[commands.Bot]):
        """Show bot uptime"""
        ...
        embed = create_embed(title="Bot Uptime", description=f"🕐 {uptime_str}")
        await ctx.send(embed=embed)
    ...
    async def about(self, ctx: commands.Context[commands.Bot]):
        ...
        embed = create_embed(
            ...
            description=(
                "A multi-platform Discord music bot that searches and "
                "plays audio from various video platforms."
            ),
        )
    ...
    async def invite(self, ctx: commands.Context[commands.Bot]):
        ...
        invite_url = discord.utils.oauth_url(
            self.bot.user.id, permissions=permissions  # type: ignore
        )

        embed = create_embed(
            title="Invite Robustty",
            description=(
                f"[Click here to invite Robustty to your server]" f"({invite_url})"
            ),
        )
        await ctx.send(embed=embed)
