...
class RumblePlatform(VideoPlatform):
    ...
    @with_retry(
        ...
        exclude_exceptions=(PlatformAuthenticationError, PlatformRateLimitError, CircuitBreakerOpenError)
    )
    async def search_videos(
        ...
        try:
            ...
            logger.info(f"Rumble search returned {len(results)} valid results for: {query}")
            return results
    ...
    async def get_stream_url(self, video_id: str) -> Optional[str]:
        ...
        if not video_id or not video_id.startswith('v'):
            ...
            raise PlatformAPIError(
                f"Invalid Rumble video ID: {video_id}",
                platform="Rumble"
            )
        ...
        try:
            ...
            def extract_info():
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        if not info:
                            logger.error("yt-dlp returned no information for Rumble video")
                        ...
                        if 'url' in info:
                            ...
                            if audio_formats:
                                ...
                                logger.debug(f"Selected Rumble audio format: {audio_formats[0].get('format_id')} (bitrate: {audio_formats[0].get('abr', 'unknown')})")
                            else:
                                # Fallback to best available format
                                valid_formats = [f for f in formats if f.get('url')]
                                if valid_formats:
                                    # Sort by quality
                                    valid_formats.sort(key=lambda f: f.get('tbr', 0) or f.get('abr', 0), reverse=True)
                                    stream_url = valid_formats[0]['url']
                                    logger.debug(f"Selected Rumble fallback format: {valid_formats[0].get('format_id')}")
                        
                        if not stream_url:
                            logger.error("No valid stream URL found in Rumble extraction result")
                            return None, "No stream URL found"
                        
                        logger.debug(f"Extracted Rumble stream URL: {stream_url[:100]}...")
            ...
            try:
                ...
                logger.error(f"Rumble stream extraction timed out for video: {video_id}")
                raise PlatformNotAvailableError(
                    "Rumble stream extraction timed out",
                    platform="Rumble"
                )
            
            # Handle errors
            if error:
                logger.error(f"Rumble stream extraction failed for video {video_id}: {error}")
                raise PlatformAPIError(
            ...
            if stream_url:
                logger.info(f"Successfully extracted Rumble stream URL for {video_id}: {stream_url[:100]}...")
                return stream_url
            else:
