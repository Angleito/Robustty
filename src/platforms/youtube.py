...
class YouTubePlatform(VideoPlatform):
        ...
        try:
            ...
            if e.resp.status == 403 and "quotaExceeded" in str(e):
                raise PlatformRateLimitError(
                    "YouTube API quota exceeded", platform="YouTube", original_error=e
                )
    ...
    async def get_stream_url(self, video_id: str) -> Optional[str]:
        ...
        try:
            ...
            cookie_paths = ["/app/cookies/youtube_cookies.json"]
    ...
    async def _validate_stream_url_async(self, url: str) -> bool:
        ...
        try:
            ...
            # Use the base class session if available, otherwise create temporary one
            ...
            # Always ensure session is closed if we created it, even during exceptions
            if own_session and session and not session.closed:
                try:
                    await session.close()
                except Exception as close_error:
                    logger.debug(f"Error closing temporary session: {close_error}")
