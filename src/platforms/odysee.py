...
class OdyseePlatform(VideoPlatform):
    ...
    async def search_videos(
        ...
        if not self.session:
            ...
            raise PlatformNotAvailableError(
                "Odysee service not initialized",
                platform="Odysee"
            )
        ...
        try:
            ...
            try:
                ...
                if response.status == 429:
                    raise PlatformRateLimitError(
                        "Odysee API rate limit exceeded",
                        platform="Odysee"
                    )
                elif response.status >= 500:
                    raise PlatformNotAvailableError(
                        f"Odysee server error ({response.status})",
                        platform="Odysee"
                    )
                ...
                raise PlatformNotAvailableError(
                    "Odysee search timed out - service may be slow",
                    platform="Odysee"
                )
            ...
            logger.info(f"Odysee search returned {len(results)} valid results for: {query}")
            return results
        ...
        try:
            ...
            try:
                ...
                if response.status == 429:
                    ...
                    error_text = await response.text()
                    logger.warning(f"Odysee video details failed: {response.status} - {error_text[:100]}...")
            ...
            return {
                ...
                "title": value.get("title", item.get("name", f"Odysee Video {video_id}")),
                "channel": item.get("signing_channel", {}).get("name", "Unknown Channel"),
                "thumbnail": value.get("thumbnail", {}).get("url", ""),
                "url": f"https://odysee.com/{item.get('canonical_url', '')}",
                "platform": "odysee",
            ...
            logger.error(f"Unexpected error getting Odysee video details for {video_id}: {e}")
            import traceback
            logger.debug(f"Full traceback: {traceback.format_exc()}")
    ...
    async def get_stream_url(self, video_id: str) -> Optional[str]:
        ...
        if not video_id or not re.match(r'^[a-f0-9]+$', video_id, re.IGNORECASE):
            ...
            raise PlatformAPIError(
                f"Invalid Odysee claim ID: {video_id}",
                platform="Odysee"
            )
        ...
        try:
            ...
            for stream_url in stream_urls:
                try:
                    ...
                    if response.status == 200:
                        logger.info(f"Successfully found Odysee stream URL: {stream_url[:100]}...")
                        return stream_url
                    elif response.status == 404:
                        logger.debug(f"Stream URL not found: {stream_url}")
                        continue
                    else:
                        logger.warning(f"Stream URL returned status {response.status}: {stream_url}")
                        continue
                    ...
                    logger.warning(f"Network error checking stream URL {stream_url}: {e}")
                    continue
                except Exception as e:
            ...
            logger.info(f"Direct stream URLs failed, trying yt-dlp for Odysee video: {video_id}")
            ...
            logger.error(f"Unexpected error getting Odysee stream URL for {video_id}: {e}")
            import traceback
            logger.debug(f"Full traceback: {traceback.format_exc()}")
            raise PlatformAPIError(
    ...
    async def _get_stream_url_with_ytdlp(self, video_id: str, video_details: Dict[str, Any]) -> Optional[str]:
        """Fallback method using yt-dlp for stream extraction"""
        try:
            import yt_dlp
            import asyncio
            
            video_url = video_details.get('url', f"https://odysee.com/$/download/{video_id}")
            ...
            try:
                ...
                logger.warning(f"yt-dlp extraction timed out for Odysee video: {video_id}")
                return None
            
            if error:
                logger.warning(f"yt-dlp extraction failed for Odysee video {video_id}: {error}")
                return None
            
            if stream_url:
                logger.info(f"Successfully extracted Odysee stream URL via yt-dlp: {stream_url[:100]}...")
                return stream_url
