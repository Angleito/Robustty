...
def bot():
    ...
    bot.searcher = Mock()
    bot.searcher.search_all_platforms = AsyncMock(return_value={
        'youtube': [
            {'id': '1', 'title': 'Test Song', 'url': 'http://test.com/1', 'platform': 'youtube'}
        ]
    })
    bot.get_audio_player = Mock()
    bot.platform_registry = Mock()
    bot.platform_registry.get_all_platforms.return_value = {'youtube': Mock(enabled=True)}
...
async def test_queue_command(music_cog, ctx, bot):
    player = Mock()
    player.get_queue = Mock(return_value=[
...
async def test_help_command(info_cog, ctx, bot):
    bot.cogs = {
        'Music': Mock(get_commands=Mock(return_value=[
